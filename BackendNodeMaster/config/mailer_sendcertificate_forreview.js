...
module.exports =async(content, sub, to, account, attachment_files, logfilename="/home/ubuntu/logs/default.log", cohorttablename, studentid) => {
    ...
    if(account['isoffice365']==undefined || account['isoffice365']==null || account['isoffice365']==0)
    {
        ...
        try{
            ...
            let updatesql= format(`UPDATE ${cohorttablename} SET certificatesendstate = 4  WHERE id=${studentid}`);
            ...
            let updatesql= format(`UPDATE ${cohorttablename} SET certificatesendstate = 5  WHERE id=${studentid}`);
            await pool.query(updatesql);
    ...
    else
    {
        ...
        {           
            try {
                ...
                if(error.response.data.error.message.indexOf("expired")!==-1)
                {
